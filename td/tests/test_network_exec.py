...
class _MockOscOut:
    def __init__(self):
        self.par = types.SimpleNamespace(hostname="", port=0)
        self.sent = []
...
def _run_onCook(monkeypatch, lights_rows, tracks_rows):
    ...
    monkeypatch.setattr(net_exec, "op", lambda name: osc_out if name == "../oscOut" else osc_out_bcast, raising=False)

    scriptOp = _MockScriptOp(
        _MockDat(lights_rows),
        _MockDat(tracks_rows),
        osc_out,
    )
